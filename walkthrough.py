...
It encapsulates metric tracking, callbacks, distribution, training loops, various input
types, and a wide variety of other training related behavior. 
This has led to the Model class containing a large volume of code that can be
intimidating to sift through.

This guide walks through a simplified model implementation in order to help you
understand what model does under the hood.
After following along with this guide you will understand how the keras model class
achieves the behavior listed above.

The SimplifiedModel can also serve as a starting point for those looking to implement
custom models or training loops.
A forkable template using the SimplifiedModel class be found on my github at
[https://github.com/lukewood/ModelWalkthrough](https://github.com/lukewood/ModelWalkthrough).
[https://github.com/lukewood/ModelWalkthrough](https://github.com/lukewood/ModelWalkthrough).

For the sake of brevity, the class written in this guide subclasses `keras.layers.Layer`
to leverage some helper functions, such as Keras' `__call__` implementation..  
"""

"""
## Core Implementation
Let's create a basic implementation that supports `compile()`, `fit()`, `predict()`, and
`eval()` before we introduce distribution strategy, metric tracking, callbacks, and other
features.

Note that the `SimplifiedModel` class operates as a `keras.Model` subclass, overriding
the `call()` method to produce predictions.
"""
...
The `compile()` method creates the optimizer, takes a loss function, and creates a
reference to a list of metrics.  These are all later used in training.

`train_step`, `predict_step`, and `eval_step` all contain the logic to perform a single
step of their corresponding methods: `fit()`, `predict()`, and `evaluate()` respectively.
Note that while `predict_step()` simply invokes call, `train_step()` and `eval_step()`
track loss and metrics.
"""

"""
The simplified model class expects a `tf.data.Dataset` as an input to `fit()`,
...
"""
This model class implements our expected behavior, but it's missing some critical logic
that `keras.Model` implements.
...
Currently we are executing our `train_step()` calls one at a time, the `train_step()`
function is not a compiled `tf.function`, and the model does not work the TensorFlow
distribution strategies.  In this section we will implement all of these performance
enhancements and net massive performance gains.
...
This function also runs the `train_step()` function using the model's provided
...
"""
Next, we will implement computation batching.  By batching computation, we reduce the
number of context transfers between the computation host and the python side callbacks. 
In the Keras model class this is done by the `steps_per_execution` parameter passed to
the `compile()` method.
...
"""
## Keras Callbacks
Keras callbacks are objects that perform actions at various stages of training.  There is
a large library of existing callbacks to handle things like:
...
"""
We can now pass any Keras callbacks to `fit()` and have it behave as expected. 
Additionally, we now get the Keras progress bar when running fit.
"""
...
"""
As you can see, `model.fit()` now runs *significantly* faster than it did before
implementing our performance enhancements.
"""

"""
# Conclusion
The `keras.Model` class contains a encapsulates set of functionality related to training.
Due to this, the lifecycle of the `keras.Model` class is significantly complex.

The `SimplifiedModel` class we implemented shows how the core functionality of the
`keras.Model` works, while still remaining terse and readable.  while the
`SimplifiedModel` class is missing a large portion of the true `keras.Model` class's
functionality, it is still as useful starting point for implementing custom training
loops that work with TensorFlow distribution strategies.

A forkable template using the `SimplifiedModel` class is available at
[https://github.com/lukewood/ModelWalkthrough](https://github.com/lukewood/ModelWalkthrough).
[https://github.com/lukewood/ModelWalkthrough](https://github.com/lukewood/ModelWalkthrough).
"""
